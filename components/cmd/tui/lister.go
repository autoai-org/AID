...
func listSolvers() {
	solvers, err := database.NewDefaultDB().Solver.Query().All(context.Background())
	utilities.ReportError(err, "cannot fetch content: solvers")
	headers := simpletable.Header{
		Cells: []*simpletable.Cell{
			{Align: simpletable.AlignCenter, Text: "#"},
			{Align: simpletable.AlignCenter, Text: "Unique ID"},
